...
class TestPolynomial(QuantityTestCase, unittest.TestCase):
    ...
    def test_polynomial_multiplication(self):
        self.assertListEqual(list((self.cubic_1 * 9.5).coef), list((9.5 * self.cubic_1).coef))
        self.assertListEqual(list((self.cubic_2 * self.polynomial_2.coef).coef),
                             list((self.cubic_2 * self.polynomial_2).coef)
                             )
        self.assertEqual((self.parabolic_1 * self.parabolic_2).x_unit, self.parabolic_1.x_unit * self.parabolic_2.x_unit)

    def test_polynomial_division(self):
        self.assertListEqual(list((self.polynomial_2 / 2).coef), [4.5, 1.0, 2.0, 0.5, 1.0])
        self.assertListEqual(list((self.cubic_2 / self.polynomial_2.coef).coef),
                             list((self.cubic_2 / self.polynomial_2).coef)
                             )
        self.assertEqual((self.parabolic_1 / self.parabolic_2).x_unit, self.parabolic_1.x_unit / self.parabolic_2.x_unit)
        self.assertListEqual(list((self.polynomial_2 // self.linear_1).coef), [2.0, 4.0, 1.0, 2.0])
        self.assertListEqual(list((self.polynomial_2 % self.linear_1).coef), [9])
        self.assertListEqual(list((self.polynomial_2 // self.linear_1 + self.polynomial_2 % self.linear_1).coef), list((self.polynomial_2 / self.linear_1).coef))



    def test_polynomial_addition(self):

        self.assertListEqual(list((self.cubic_1 + 5).coef), list((5 + self.cubic_1).coef))
    ...
    def test_correct_units_after_math(self):
        ...
        self.assertEqual(
            (self.linear_1 - self.linear_2).y_unit, self.linear_1.y_unit
        )
        self.assertEqual(
