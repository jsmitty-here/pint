...
class Polynomial(p.Polynomial):
    ...
    def __mod__(self, other):
        new_coefficients, x_unit, y_unit = self._division_coef_and_units(other)
        return self.__class__(new_coefficients[1], x_unit, y_unit)

    def __floordiv__(self, other):
        new_coefficients, x_unit, y_unit = self._division_coef_and_units(other)
        return self.__class__(new_coefficients[0], x_unit, y_unit)

    def _division_coef_and_units(self, other):
        x_unit, y_unit = self.x_unit, self.y_unit
        if isinstance(other, self.__class__):
            x_unit /= other.x_unit
            y_unit /= other.y_unit
            other_coefs = other.coef
        else:
            other_coefs = super(Polynomial, self)._get_coefficients(other)
        return p.polydiv(self.coef, other_coefs), x_unit, y_unit
